...
\item Progress toward that goal or target over the course of the day; and
...
\fbox{\includegraphics[width=1.25in]{./images/cont1/fig8c}}
	...
	\end{subfigure}
	\caption[Evolution of serving size entry]{Evolution of serving size specification to make it more touch-friendly.}\label{fig:fig8}
\end{figure}
...
\caption[Create manual entry ]{Create manual entry for the food diary when the database does not have the desired food. }
\label{fig:fig9}
\end{center}
\end{figure}
%\end{SCfigure}
...
Adopting the commercially available NPKB resulted in a much larger number of foods available to the user. While the USDA database had the user challenge of making it difficult to distinguish between many similar unprocessed foods and relatively few processed or manufactured foods, the NPKB had many more processed and manufactured foods to choose from, which users sometimes found overwhelming. 
...
\fbox{\includegraphics[width=1.25in]{./images/cont1/fig11d}}
	...
	\caption[Final BALANCE screens]{Final BALANCE non-food diary screens.}\label{fig:fig11}
\end{figure}
...
\caption[24-hr recall software]{Software to support the 24-hr recall using the same database and queries as the mobile software.  }
\label{fig:fig12}
...
\begin{table}[htbp]
  \centering
  \caption{Time frame definitions}
    \begin{tabular}{ll}
    \toprule
...
\caption[Daily app starts (mean)]{Mean app starts for each day }
...
\caption{Number of entries eaten and created over time}
...
\caption{All measures, Participant 120}
\label{fig:chart19}
